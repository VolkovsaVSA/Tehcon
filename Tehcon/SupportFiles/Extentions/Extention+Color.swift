...
public extension Color {
    ...
    static var systemGray5AllPlatform: Color {
        #if os(iOS)
        return Color.systemGray5
        #endif
        
        #if os(macOS)
        return Color.init(NSColor.windowBackgroundColor)
        #endif
    }
//    #if os(iOS)
    static let dynamicColor = Color(UIColor { traitCollection in
        return traitCollection.userInterfaceStyle == .dark ? .black : .blue
    })
    static let mainBackgroundColor = Color(UIColor { traitCollection in
        return traitCollection.userInterfaceStyle == .dark ? #colorLiteral(red: 0.5568627715, green: 0.3529411852, blue: 0.9686274529, alpha: 1) : .white
    })
//    #endif
//    #if os(macOS)
//    static let dynamicColor = Color(UIColor { traitCollection in
